...
제출 당시 소스를 보존한 아카이브입니다. 아래 실행 결과는 제출 PDF에서 확인한 기록이며, 저장소 정리 과정에서 xv6를 다시 실행해 얻은 결과는 아닙니다.
...
제출본 Makefile은 그대로 유지했습니다. Phase 1의 `test_1`/`test_2`는 기본 `UPROGS`에 없고, Phase 3의 `_test_currency`는 주석 처리되어 있습니다. 해당 프로그램을 직접 실행하려면 각 Makefile의 `UPROGS`에 등록하고 다시 빌드해야 합니다. Phase 1의 기본 `test`는 Hello World 프로그램으로, 보고서의 두 시험과 다릅니다.

현대 GCC에서 제출 당시와 다른 경고나 빌드 문제가 발생할 수 있습니다. 이번 정리에서는 Unix 유틸리티의 GCC 정적 문법 검사를 수행했으며, xv6 빌드·QEMU 실행은 수행하지 않았습니다.

## 자료 출처 및 기여

- 시스템 콜·스케줄링: `202128519.zip`의 `phase1`~`phase3`, 같은 압축파일의 `202128519.pdf`.
- 커널 스레드: `202020937.tar.gz`의 `ECE359_pa`, `202020937.pdf`.
- Unix 유틸리티: `wzip.pdf`, `wunzip.pdf`의 소스 코드. `HW1_unix-utilities.tar`는 시험 구성 확인에 사용했습니다.
